...
use bevy_ui::{
    widget::TextNodeFlags, ComputedNode, ComputedUiRenderTargetInfo, ComputedUiTargetCamera,
    ContentSize, Node, UiGlobalTransform, UiScale,
};
use bevy_window::{Ime, PrimaryWindow, Window, WindowRef};
...
/// Marks an [`EditableText`] whose keyboard edits are routed by its owning UI.
///
/// Pointer selection, focus, IME display, and text rendering remain handled by this plugin.
/// Only the default global keyboard-to-`TextEdit` conversion is skipped.
#[derive(Component, Clone, Default, Reflect)]
#[reflect(Component)]
pub struct ManualKeyboardInput;
...
fn on_focused_keyboard_input(
    ...
    mut query: Query<(&mut EditableText, Has<ManualKeyboardInput>)>,
    keys: Res<ButtonInput<Key>>,
) {
    let Ok((mut editable_text, manual_keyboard_input)) =
        query.get_mut(keyboard_input.focused_entity)
    else {
        return; // Focused entity is not an EditableText, nothing to do
    };
    if manual_keyboard_input {
    ...
    match (mod_flags, &keyboard_input.input.logical_key) {
        ...
        (NONE | SHIFT, Key::Character(_)) | (NONE, Key::Space) => {
            if let Some(text) = keyboard_input_text(&keyboard_input.input) {
                queue_edit(TextEdit::Insert(text));
            }
        }
...
/// Winit may omit `KeyboardInput::text` for character keys received by a secondary
/// native window. In that case the logical key is still the correct insertion text.
fn keyboard_input_text(input: &KeyboardInput) -> Option<smol_str::SmolStr> {
    input
        .text
        .as_ref()
        ...
        .or_else(|| match &input.logical_key {
            Key::Character(character) if !character.is_empty() => Some(character.clone()),
            Key::Space => Some(" ".into()),
            _ => None,
        })
}
...
fn update_ime_position(
    ...
    camera_targets: Query<&RenderTarget, With<Camera>>,
    ...
    let Ok((editable_text, node, transform, target, target_camera, text_scroll)) =
    ...
    let Some(window_entity) =
        ui_target_window(target_camera.get(), &camera_targets, primary_window)
    else {
        return;
    };
...
/// Enables IME on the native window that renders the focused [`EditableText`].
///
...
fn listen_for_ime_input_when_text_input_focused(
    input_focus: Res<InputFocus>,
    editable_text_query: Query<Option<&ComputedUiTargetCamera>, With<EditableText>>,
    camera_targets: Query<&RenderTarget, With<Camera>>,
    primary_window: Query<Entity, With<PrimaryWindow>>,
    ...
    let primary_window = primary_window.single().ok();
    let focused_window = input_focus
        .get()
        .and_then(|entity| editable_text_query.get(entity).ok())
        .and_then(|target_camera| {
            ui_target_window(
    ...
    for (entity, mut window) in &mut windows {
        let should_enable = focused_window == Some(entity);
...
/// Resolves a UI camera to the native window receiving text and IME events.
fn ui_target_window(
    ...
    render_target_window(camera_targets.get(target_camera).ok()?, primary_window)
}

fn render_target_window(
...
mod tests {
    ...
    #[test]
    fn ime_window_follows_the_ui_camera_render_target() {
        let mut world = World::new();
        let primary = world.spawn_empty().id();
        let secondary = world.spawn_empty().id();

        assert_eq!(
            render_target_window(&RenderTarget::Window(WindowRef::Primary), Some(primary)),
            Some(primary)
        );
        assert_eq!(
    ...
    #[test]
    fn character_input_falls_back_to_the_logical_key_when_text_is_missing() {
        ...
        assert_eq!(keyboard_input_text(&input).as_deref(), Some("2"));
    }
}
