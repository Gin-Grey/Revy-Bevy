...
pub struct GltfLoaderSettings {
    ...
    /// If true, scenes with one clear multi-mesh subject will only spawn that
    /// subject and any separate roots required by its skins.
    ///
    /// This is intended for model slots that should import one model rather
    /// than every staging object exported in the glTF scene. Ambiguous scenes
    /// retain all of their roots.
    #[serde(default)]
    pub load_main_model_only: bool,
...
impl GltfLoader {
    ...
    pub async fn load_gltf<'a, 'b, 'c>(
        ...
        for scene in gltf.scenes() {
            ...
            let model_roots = settings
                .load_main_model_only
                .then(|| main_model_root_indices(&scene, &gltf.document))
                .flatten();
            ...
            let world_root_id = world
                ...
                .with_children(|parent| {
                    for node in scene.nodes() {
                        if model_roots
                            .as_ref()
                            .is_some_and(|roots| !roots.contains(&node.index()))
                        {
                            continue;
                        }
            ...
            #[cfg(feature = "bevy_animation")]
            {
                ...
                for node in scene.nodes() {
                    if animation_roots.contains(&node.index()) {
                        if let Some(&entity) = node_index_to_entity_map.get(&node.index()) {
...
fn main_model_root_indices(
    ...
    for root in &roots {
        ...
        let mut skins = HashSet::default();
        collect_model_subtree(root, &mut nodes, &mut mesh_count, &mut skins);
        root_stats.push((nodes, mesh_count, skins));
    }

    let largest_mesh_count = root_stats.iter().map(|(_, count, _)| *count).max()?;
    let total_mesh_count: usize = root_stats.iter().map(|(_, count, _)| *count).sum();
    let mut largest_roots = root_stats
        .iter()
        .enumerate()
        .filter_map(|(index, (_, count, _))| (*count == largest_mesh_count).then_some(index));
    let largest_root = largest_roots.next()?;

    // A single mesh is often just one part of a legitimate multi-root model.
    // Only collapse the scene when one hierarchy is both unique and dominant.
    if largest_mesh_count < 2
        || largest_roots.next().is_some()
        || largest_mesh_count.saturating_mul(2) < total_mesh_count
    {
        return None;
    }

    let subject_skins = &root_stats[largest_root].2;
    let mut required_joints: HashSet<usize> = HashSet::default();
    for skin in document
        .skins()
        .filter(|skin| subject_skins.contains(&skin.index()))
    {
        required_joints.extend(skin.joints().map(|joint| joint.index()));
    }

    let mut selected_roots: HashSet<usize> = HashSet::default();
    selected_roots.insert(roots[largest_root].index());
    if !required_joints.is_empty() {
        for (root, (nodes, _, _)) in roots.iter().zip(&root_stats) {
            if nodes.iter().any(|node| required_joints.contains(node)) {
                selected_roots.insert(root.index());
            }
        }
    }
    Some(selected_roots)
...
fn collect_model_subtree(
    node: &Node<'_>,
    nodes: &mut HashSet<usize>,
    mesh_count: &mut usize,
    skins: &mut HashSet<usize>,
) {
    ...
    if let Some(skin) = node.skin() {
        skins.insert(skin.index());
    }
    for child in node.children() {
        collect_model_subtree(&child, nodes, mesh_count, skins);
    }
}
...
mod test {
    ...
    use super::main_model_root_indices;
    ...
    #[test]
    fn main_model_only_keeps_subject_and_its_separate_skeleton_root() {
        let gltf = gltf::Gltf::from_slice_without_validation(
            br#"
{
    "asset": { "version": "2.0" },
    "nodes": [
        { "name": "Background", "mesh": 0 },
        { "name": "Character", "children": [2, 3] },
        { "name": "Body", "mesh": 1, "skin": 0 },
        { "name": "Equipment", "mesh": 2 },
        { "name": "Armature", "children": [5] },
        { "name": "Bone" }
    ],
    "meshes": [
        { "primitives": [] },
...
"#,
        ...
        .unwrap();
        let scene = gltf.document.scenes().next().unwrap();

        let mut roots: Vec<_> = main_model_root_indices(&scene, &gltf.document)
            .unwrap()
            .into_iter()
            .collect();
        roots.sort_unstable();
