...
impl WinitWindows {
    ...
    pub fn create_window(
        ...
        #[cfg(target_os = "windows")]
        let winit_window_attributes = with_embed_parent(winit_window_attributes);
...
#[expect(
    unsafe_code,
    reason = "the editor supplies a live HWND that remains valid for the player lifetime"
)]
fn with_embed_parent(
    mut attributes: winit::window::WindowAttributes,
) -> winit::window::WindowAttributes {
    use core::num::NonZeroIsize;
    use winit::raw_window_handle::{RawWindowHandle, Win32WindowHandle};

    let Some(parent) = revy_embed_env("REVY_EMBED_PARENT_HWND", "ARISNA_EMBED_PARENT_HWND")
    ...
    if let Some((x, y, width, height)) = embedded_window_rect() {
        attributes = attributes
            .with_position(PhysicalPosition::new(x, y))
            .with_inner_size(winit::dpi::PhysicalSize::new(width, height));
    }
    let parent = RawWindowHandle::Win32(Win32WindowHandle::new(parent));
    unsafe {
        attributes
...
#[cfg(target_os = "windows")]
fn embedded_window_rect() -> Option<(i32, i32, u32, u32)> {
    let x = revy_embed_env("REVY_EMBED_X", "ARISNA_EMBED_X")?
        .parse()
        .ok()?;
    let y = revy_embed_env("REVY_EMBED_Y", "ARISNA_EMBED_Y")?
        .parse()
        .ok()?;
    let width = revy_embed_env("REVY_EMBED_WIDTH", "ARISNA_EMBED_WIDTH")?
        .parse::<u32>()
        .ok()?
        .max(2);
    let height = revy_embed_env("REVY_EMBED_HEIGHT", "ARISNA_EMBED_HEIGHT")?
        .parse::<u32>()
        .ok()?
        .max(2);
...
#[cfg(target_os = "windows")]
fn revy_embed_env(primary: &str, legacy: &str) -> Option<String> {
    std::env::var(primary)
        .ok()
