...
use bevy_reflect::{PartialReflect, TypeRegistryArc};
...
/// A component template whose concrete component type is selected from reflection data at runtime.
///
/// This is used by data-driven scene formats such as `.bsn`. The reflected value may be partial:
/// missing fields are populated using the same `Default` / `FromWorld` reflection fallback used by
/// [`ReflectComponent::insert`].
pub struct ReflectedComponentTemplate {
    value: Box<dyn PartialReflect>,
    reflect_component: ReflectComponent,
    registry: TypeRegistryArc,
}

impl ReflectedComponentTemplate {
    /// Creates a runtime component template.
    pub fn new(
        value: Box<dyn PartialReflect>,
        reflect_component: ReflectComponent,
        registry: TypeRegistryArc,
    ...
    /// Returns the reflected partial component value stored by this template.
    pub fn value(&self) -> &dyn PartialReflect {
        &*self.value
    }
}

impl ErasedComponentTemplate for ReflectedComponentTemplate {
    unsafe fn apply(
        &self,
        context: &mut TemplateContext,
        bundle_writer: &mut BundleWriter,
        ...
        // SAFETY: `ErasedComponentTemplate::apply` requires the writer and context to use the
        // same world. `world_mut` is only used for component construction and registration, so
        // the target entity's location is not accessed while the world is borrowed.
        let world = unsafe { context.entity.world_mut() };
        // SAFETY: the invariant above is exactly the contract of `push_to_bundle`.
        unsafe {
            self.reflect_component
                .push_to_bundle(world, &*self.value, &registry, bundle_writer)
        };
        Ok(())
    }
