...
pub struct ReflectComponentFns {
    ...
    /// Function pointer implementing [`ReflectComponent::push_to_bundle()`].
    pub push_to_bundle:
        unsafe fn(&mut World, &dyn PartialReflect, &TypeRegistry, &mut BundleWriter),
}
...
impl ReflectComponent {
    ...
    /// Builds this component from reflection data and pushes it into a dynamic bundle.
    ///
    /// This is primarily intended for data-driven scene and scripting systems where the
    /// concrete component type is only known at runtime.
    ///
    /// # Safety
    ///
    /// `bundle_writer` must only contain components registered in `world`, and its next
    /// [`BundleWriter::write`] call must target an entity in that same `world`.
    pub unsafe fn push_to_bundle(
        &self,
        world: &mut World,
        component: &dyn PartialReflect,
        registry: &TypeRegistry,
        ...
        unsafe { (self.0.push_to_bundle)(world, component, registry, bundle_writer) };
    }
...
impl<C: Component + Reflect + TypePath> FromType<C> for ReflectComponent {
    fn from_type() -> Self {
        ...
        ReflectComponent(ReflectComponentFns {
            ...
            push_to_bundle: |world, reflected_component, registry, bundle_writer| {
                let component =
                    from_reflect_with_fallback::<C>(reflected_component, world, registry);
                let mut components = world.components_registrator();
                // SAFETY: the function caller guarantees that `bundle_writer` belongs to
                // this world, and the component type and registration are both `C`.
